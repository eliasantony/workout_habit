...
Future<void> backgroundCallback(Uri? uri) async {
  ...
  try {
    ...
    final logFile = File('/data/data/com.example.hydrohabit.hydro_habit/cache/bg_debug_log.txt');
    logFile.writeAsStringSync('BACKGROUND CALLED with URI: $uri\n', mode: FileMode.append);
  } catch(e) {
  ...
  try {
    ...
    if (uri.scheme == 'home_widget' && (uri.host == 'add_water' || uri.path.contains('add_water'))) {
      ...
      if (amount != null && amount > 0) {
        ...
        await prefs.setString('bg_log', 'SUCCESSFULLY ADDED $amount ml. URI: $uri');
      } else {
